...
class Module implements ModuleInterface
{
        ...
        switch ($message['arguments'][0]) {
            case 'load':
                ...
                switch ($module) {
                    //AlreadyLoaded
                    case 'AL':
                        $wrapper->Message->reply('The Module `' . $message['arguments'][1] .
                            '` is already loaded.');
                        break;

                    //Loaded
                    case 'L':
                        $wrapper->Message->reply('Module `' . $message['arguments'][1] .
                            '` loaded successfully.');
                        break;

                    //NotFound
                    case 'NF':
                        $wrapper->Message->reply('The Module `' . $message['arguments'][1] . '` was not found.');
                        break;
                }
                break;
            ...
            case 'unload':
                ...
                if (Configure::read('debug') === false) {
                    $wrapper->Message->reply('You can\'t unload a Module when the debug is `false`.');
                    break;
                }
                ...
                if ($module === 'AU') {
                    ...
                    $wrapper->Message->reply('Module `' . $message['arguments'][1] . '` unloaded successfully.');
                }
                break;
            ...
            case 'reload':
                //Prevent for loading a file in the memory for nothing.
                if (Configure::read('debug') === false) {
                    $wrapper->Message->reply('You can\'t reload a Module when the debug is `false`.');
                    break;
                }
            ...
            case 'time':
                ...
                if ($time === false) {
                    $wrapper->Message->reply('This Module is not loaded.');
                    break;
                }
                ...
                $wrapper->Message->reply('The Module `' . Inflector::camelize($message['arguments'][1]) .
            ...
            case 'loaded':
                ...
                $wrapper->Message->reply('Modules loaded : `' . Inflector::camelize($modules) . '`.');
                break;

            default:
                $wrapper->Message->reply(Command::unknown($message));
        }
    }
    ...
    protected function reloadModule(Wrapper $wrapper, $module)
    {
        ...
        switch ($moduleStatus) {
            //AlreadyUnloaded
            case 'AU':
                $wrapper->Message->reply('The Module `' . $module . '` doesn\'t exist and cannot be reloaded.');
                break;

            //AlreadyLoaded
            case 'AL':
                $wrapper->Message->reply('The Module `' . $module . '` is already loaded.');
                break;

            //Loaded
            case 'L':
                $wrapper->Message->reply('Module `' . $module . '` reloaded successfully.');
                break;

            //NotFound
            case 'NF':
                $wrapper->Message->reply('Failed to reload the Module `' . $module . '`.');
                break;
        }
    }
}
