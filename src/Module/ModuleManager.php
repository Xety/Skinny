...
class ModuleManager implements ArrayAccess, Countable
{
     ...
     * @var mixed
     */
    protected mixed $prefixArgument = null;
     ...
     */
    public function __construct(array $priorities = [], ?LoggerInterface $logger = null)
    {
        ...
        $this->logger->info('ModuleManager initialized', [
            'modules_count' => count($this->loadedModules)
        ]);
    }
     ...
     * work is done.
     *
     * Includes error handling to prevent one module from breaking others.
     *
    ...
    public function __call(string $method, array $arguments): bool
    {
        ...
        foreach ($this->loadedModules as $moduleName => $module) {
            ...
            try {
                $result = call_user_func_array([$module['object'], $method], $arguments);

                if ($result === self::STOP) {
                    $this->logger->debug("Module {$moduleName} stopped propagation for {$method}");
                    break;
                }
            } catch (\Throwable $e) {
                $this->logger->error("Error in module {$moduleName}::{$method}: {$e->getMessage()}", [
                    'exception' => $e,
                    'module' => $moduleName,
                    'method' => $method,
    ...
    public function load($module, array $config = [])
    {
        ...
        $className = $namespace . DS . 'Module' . DS . 'Modules' . DS . $module;
     ...
     *
     * @param mixed $module The module to get.
     *
     * @return mixed
     */
    public function offsetGet(mixed $module): mixed
    {
        $module = Inflector::camelize($module);
     ...
     *
     * @param mixed $module The module to check.
     *
     * @return bool
     */
    public function offsetExists(mixed $module): bool
    {
        $module = Inflector::camelize($module);
     ...
     *
     * @param mixed $offset The offset of the module.
     * @param mixed $module The module to create.
     *
     * @throws \RuntimeException
     *
     * @return void
     */
    public function offsetSet(mixed $offset, mixed $module): void
    {
     ...
     *
     * @param mixed $module The module to unlod.
     *
     * @return void
     */
    public function offsetUnset(mixed $module): void
    {
        $module = Inflector::camelize($module);
