...
class Plugin
{
    ...
    public static function bootstrap($plugin)
    {
        ...
        // Auto-load commands file if it exists (DRY approach)
        static::autoloadCommands($plugin);

        // Load custom bootstrap.php if it exists
        if ($config['bootstrap'] === true) {
            $bootstrapFile = $config['configPath'] . 'bootstrap.php';
            if (is_file($bootstrapFile)) {
                return static::includeFile($bootstrapFile, $config['ignoreMissing']);
            }
        }
    ...
    /**
     * Automatically loads the commands configuration file for a plugin if it exists.
     * This eliminates the need for repetitive bootstrap.php files that only load commands.
     *
     * @param string $plugin Name of the plugin.
     *
     * @return bool True if commands were loaded, false otherwise.
     */
    public static function autoloadCommands($plugin)
    {
        $config = static::$plugins[$plugin];
        $commandsFile = $config['configPath'] . 'commands.php';
        ...
        if (is_file($commandsFile)) {
            try {
                ...
                // Log error but don't die - allow plugin to continue loading
                error_log("Failed to load commands for plugin {$plugin}: " . $e->getMessage());
                return false;
            }
        }
